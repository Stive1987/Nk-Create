...
if ($visiteur >= $level_access && $level_access > -1)
{
	...
	function editJquery($id) {
        global $visiteur, $user, $nuked;
		

            $dbsForum = '  SELECT txt, auteur, forum_id
                            FROM '.FORUM_MESSAGES_TABLE.'
                            WHERE id = "'.$id.'"';
            $dbeForum = mysql_query($dbsForum);
            list($txt, $auteur, $forum_id) = mysql_fetch_array($dbeForum);
			...
			if ($auteur == $user[2] || $visiteur >= admin_mod("Forum")) {
				$t['edit'] = 'Editer par' . $user[2];
				$t['texte'] =  $txt;
				$t['id'] = $id;

			} else {
				$t['edit'] = '  <div class="reply-obj paddingNone">
									<div class="t1">Edition rapide</div>
									<div class="t2">Editer ce sujet</div>
									<div class="box01">
										<div class="error-obj">
											<div class="box01">
												<span class="s1">ERREUR</span>Vous n\'avez pas acc&egrave;s.
											</div>
										</div>
									</div>
								</div>';
    ...
    function quickReply($titre, $txt, $usersig, $emailnotify, $thread_id, $forum_id) {
        global $user, $nuked;
...
?>
                    <div class="error-obj">
                        <div class="box01">
                            <span class="s1">Attention ! </span>Pour participer, vous devez &ecirc;tre connect&eacute;
                        </div>
                    </div>
<?php
			} else if(empty($txt)) {
?>
                    <div class="error-obj">
                        <div class="box01">
                            <span class="s1">Attention ! </span>Vous devez &eacute;crire un texte
                        </div>
                    </div>
<?php
			} else {

            $txt = secu_html(nkHtmlEntityDecode($txt));
            $txt = icon($txt);
            $txt = mysql_real_escape_string(stripslashes($txt));
						
            $dbsForum = '  SELECT titre
                            FROM '.FORUM_MESSAGES_TABLE.'
                            WHERE thread_id = "'.$thread_id.'" 
                            AND forum_id = "'.$forum_id.'"';
            $dbeForum = mysql_query($dbsForum);
            list($titre) = mysql_fetch_array($dbeForum);

            $dbiForumSendQuick = '   INSERT INTO '.FORUM_MESSAGES_TABLE.' ( 
								              `id` 
								              , `titre` 
								              , `txt` 
								              , `date` 
                                  ...
                                  VALUES ( "" , "RE : ' . $titre . '"
											  ...
											  , "' . (int) $forum_id . '"
											  , "" )';
            $dbeForumSend = mysql_query($dbiForumSendQuick);
			
?>
                    <div class="success-obj">
                        <div class="box01">
                            <span class="s1">Success</span>Votre r&eacute;ponse a &eacute;t&eacute; enregistr&eacute;e 
                        </div>
                    </div>
<?php
			}
...
<?php
            $dbsForum = '  SELECT titre
                            ...
                            WHERE thread_id = "'.$thread_id.'"';
            $dbeForum = mysql_query($dbsForum);
			$dbcForum = mysql_num_rows($dbeForum);

            if ($dbcForumMsg > $nuked['mess_forum_page'])
            {
                $topicpages = $dbcForumMsg / $nuked['mess_forum_page'];
                $topicpages = ceil($topicpages);

                $url = "index.php?file=Forum&page=viewtopic&forum_id=" . $_REQUEST['forum_id'] . "&thread_id=" . $thread_id . "&p=" . $topicpages;
            }
            else
            {
                $url = "index.php?file=Forum&page=viewtopic&forum_id=" . $_REQUEST['forum_id'] . "&thread_id=" . $thread_id;
	...
	function quickEdit($txt, $id, $edit) {
        global $visiteur, $user, $nuked;
?>
			<div id="fullwidth_main" class="container">
				<div class="singal_main">
<?php
            ...
            $dbsForum = '  SELECT auteur, thread_id, forum_id
                           FROM '.FORUM_MESSAGES_TABLE.'
                           WHERE id = "'.$id.'"';
            $dbeForum = mysql_query($dbsForum);
            list($auteur, $thread_id, $forum_id) = mysql_fetch_array($dbeForum);
			...
			if ($auteur == $user[2] || $visiteur >= admin_mod("Forum") || $administrator == 1)
			{
				$txt = secu_html(nkHtmlEntityDecode($txt));
				$txt = icon($txt);
				$txt = mysql_real_escape_string(stripslashes($txt));
			
				$dbuForumMsgUpdate = ' UPDATE '.FORUM_MESSAGES_TABLE.' 
									   SET txt = "'.$txt.'", edition = "'.$edit.'"
									   WHERE id = "'.$id.'"';
				$dbeForumMsgUpdate = mysql_query($dbuForumMsgUpdate);

?>
                    <div class="success-obj">
                        <div class="box01">
                            <span class="s1">Success</span>Votre r&eacute;ponse a &eacute;t&eacute; editer 
                        </div>
                    </div>
				</div>
			</div>
...
<?php
    ...
    {		
	    case"quickEdit":
            quickEdit($_REQUEST['txt'], $_REQUEST['id'], $_REQUEST['edit']);
            break;
			
        case"quickReply":
            quickReply($_REQUEST['titre'], $_REQUEST['txt'], $_REQUEST['usersig'], $_REQUEST['emailnotify'], $_REQUEST['thread_id'], $_REQUEST['forum_id']);
            break;

        case"editJquery":
            editJquery($_REQUEST['id']);
            break;
